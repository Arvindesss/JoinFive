...
class PlayableCirclesCalculatorTest {

    @Test
    void calculateCirclesFromPossibleLines() {
        //Given a new JFGrid with a drawn cross
        JFGrid jfGrid = new JFGrid();
        fillGrid(jfGrid);
        JoinFiveGridDrawer.drawCrossOnEmptyGrid(jfGrid);
        //When we calculate all playable circles from the possibles lines
        List<JFCircle> circles = PlayableCirclesCalculator.calculateCirclesFromPossibleLines(jfGrid, JFGameType.FIVE_D);
        //Then we expect this result
        assertEquals(28, circles.size());
    }
